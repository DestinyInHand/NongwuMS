...
// var HOST = '127.0.0.1';
var HOST = '127.0.0.1';
var DATABASE = config.DATABASE_NAME;
//
var user = config.MYSQL_USER;
var password = config.MYSQL_PASSWD;
...
var selectAsList = function(tag,lastupload,sinceupload,callback) {
	...
	if(tag == 0){
		if(lastupload == 0 && sinceupload == 0) {
			sql = "select id, title, image, content, uploadtime from event order by uploadtime desc limit "+range;
		...
		if(lastupload != 0 && sinceupload == 0) {
			sql = "select id, title, image, content, uploadtime from event where uploadtime > "+lastupload+" order by uploadtime desc limit "+range;
		...
		if(lastupload == 0 && sinceupload != 0) {
			sql = "select id, title, image, content, uploadtime from event where uploadtime < "+sinceupload+" order by uploadtime desc limit "+range;
	    ...
	    connection.query(sql, function(err, results){
	    	...
	    	if(tag!=0){
		    	for (var i = results.length - 1; i >= 0; i--) {
		    		...
		    		results[i].content = content.replace(/<[^>]*>/g,"").replace(/&quot;/g,"").replace(/&apos;/g,"");
		    		// console.log("---content---"+results[i].content);
		    		...
		    		// console.log("--id--flag-title---"+results[i].id+"--"+flag+"--"+results[i].title)
...
var selectFromRegionByName = function(info,callback){
	var sql = "select * from region where name = '"+info['name']+"' and super = "+info['superID'];
	console.log("---"+sql)
	// client.query(sql, function(err, resluts){
	// 	callback(err, resluts);
	// });
...
//修改村庄
var adminRegionModifyOne = function(id, name, callback){
	var sql = " update region set name = '"+name+"' where id = "+id;
	console.log("sql--"+sql);
	pool.getConnection(function(err,connection){
	    if (err) {
	      console.log(err.message);
...
var adminEventSelectAll = function(id, callback){
	var sql = "select id, title, regionid, categoryid, uploadtime from event where regionid =" + id+" order by uploadtime desc";
	// client.query(sql, function(err, resluts){
	// 	callback(err, resluts);
 //    });
 	pool.getConnection(function(err,connection){
...
//获取最新版本号
var adminVersionSelectLastestVersion = function(callback){
	var sql = "SELECT * FROM appinfo ORDER BY id DESC;";
	pool.getConnection(function(err, connection){
	    if (err) {
	      console.log(err.message);
	      connection.release();
...
//添加新应用
var adminVersionAddNewApp = function(version, callback){
	var sql = "INSERT INTO appinfo (savepath, VERSION ,uploadtime) VALUES('"+version['savepath']+"', '"+version['version']+"', '"+ Date.parse( new Date())+"');";
	pool.getConnection(function(err, connection){
	    if (err) {
	      console.log(err.message);
	      connection.release();
...
var selectVillageFromRegion = function(towns,callback){
	var sql = "select * from region where super <> 0 order by id desc";
	// client.query(sql,function(err,results){
	// 	callback(err,results);
	// });
	pool.getConnection(function(err,connection){
...
exports.adminVersionAddNewApp = adminVersionAddNewApp;
...
exports.adminVersionSelectLastestVersion = adminVersionSelectLastestVersion;
...
exports.adminRegionModifyOne = adminRegionModifyOne;
