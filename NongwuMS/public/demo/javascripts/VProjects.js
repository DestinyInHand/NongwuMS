$(function(){
  ...
  function loaddata(page){
    ...
    var url = PINGYUANIP+"/web_gov/project?page="+page;

    $.getJSON(url , function(result){
      $('#policy_tbody').html('');
      $.each(result.projects,function(i,item){

        $('#policy_tbody').append("<tr><td>"+(i+1)+"</td><td>"+
        item.title+"</td><td>"+parseTime(item.uploadtime)+"</td><td><a href='"+"./VArticleDetail.html?type="+2+"&id="+item.id+ "' target=_blank"+">查看</a></td></tr>")
      });
